...
class MF(object):
    ...
    def fit(self):
        ...
        for it in range(self.max_iter):
            ...
            if (it + 1) % self.print_every == 0:
                print ('iter = {}  loss = {} '.format( it + 1, self.loss()))
    ...
    def pred_for_user(self, user_id):
        ...
        items, evals = zip(*(sorted(predicted_ratings, key=lambda x: x[1], reverse=True)[0:10]))
        return items
    def evaluate(self, data_test):
        ...
        for i in range(0,self.n_users):
            # print(i)
            predict = 0
            items_pre = self.pred_for_user(i)
            for j in data_test:
                if j[0] == i:
                    # print(j[0])
                    if j[1] in items_pre:
...
ratings_test = pd.read_csv('./data/test.csv', names=r_cols, encoding='latin-1')
# print(ratings_base)
